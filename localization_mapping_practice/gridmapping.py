...
def cal_grid_map_config(ox, oy, xy_resolution):
    """
    Calculate the size, and the maximum distances according to the
    measurement center
    """
...
def init_flood_fill(center_point, obstacle_points, xy_points, min_coord, xy_resolution):
    ...
    for (x, y) in zip(ox, oy):
        # x,y coord. of the occupied area
...
def generate_ray_casting_grid_map(ox, oy, xy_resolution, breshen=True):
    min_x, min_y, max_x, max_y, x_w, y_w = cal_grid_map_config(ox, oy, xy_resolution)

    occupancy_map = np.ones((x_w, y_w)) / 2     # 0.5 로 채움.
    center_x = int(round(-min_x / xy_resolution))   # center x coord. of the grid map
    center_y = int(round(-min_y / xy_resolution))   # center y coord. of the grid map
    ...
    if breshen:
        for (x, y) in zip(ox, oy):
            ...
            occupancy_map[ix][iy] = 1.0     # 센서가 인식한 위치, occupied area 1.0
...
def main():
    ...
    ang, dist = file_read("./lidar01.csv")   # outputs : np.array(angles), np.array(distnaces) 
    ox = np.sin(ang) * dist
    oy = np.cos(ang) * dist
