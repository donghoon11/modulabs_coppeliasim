...
class Grid():
    def __init__(self):
        ...
        self.grid_plot = self.ax.pcolor(self.R, self.P, self.grid, cmap="gray", shading='auto')
        self.robot_plot = self.ax.plot([], [], 'go', markersize=10)
        self.scan_plots = [self.ax.plot([], [], '--b')[0] for _ in range(5)]
    ...
    def visualize(self, loc, scan):
        ...
        # 그리드 업데이트
        grid = -self.grid + 5
        self.grid_plot.set_array(grid.ravel())  # set_array 를 통해 기존 객체를 갱신
        self.grid_plot.set_clim(vmin=0, vmax=10)    # 색상 범위 설정
        ...
        for i, data in enumerate(scan):
            ...
            xi = rx + dist * np.cos(ti)
            yi = ry + dist * np.sin(ti)
            self.scan_plots[i].set_data([rx, xi], [ry, yi])
